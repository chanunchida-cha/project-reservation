...
const ReservRoundTodayData = observer(() => {
  const history = useHistory();
  const { id } = useParams();
  const [selected, setSelected] = useState();
  const [searchText, setSearchText] = useState("");
  useEffect(() => {
    const getRoundToday = async () => {
      await reservStore.getRoundToday(id);
    };
    getRoundToday();
  }, []);
...
export default ReservRoundTodayData;
