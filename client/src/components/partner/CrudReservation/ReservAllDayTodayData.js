...
const ReservAllDayTodayData = observer(() => {
  const history = useHistory();
  const { id } = useParams();
  const [selected, setSelected] = useState();

  useEffect(() => {
    const getAlldayToday = async () => {
      await reservStore.getAlldayToday(id);
    };
    getAlldayToday();
  }, []);
  ...
  // console.log(reservStore.allDayReservToday);
            ...
            <div className="-mx-4 sm:-mx-8 px-4 sm:px-8 py-4 overflow-x-auto">
              <div className="inline-block min-w-full shadow-md rounded-lg overflow-hidden">
                <table className="min-w-full leading-normal">
                  ...
                  <tbody>
                    {reservStore.allDayReservToday.map((reserv, index) => {
                      return (
                        <tr key={index}>
                          <td className="px-1 py-2 border-b border-gray-200 bg-white text-sm text-center">
                            <p className="text-gray-900 whitespace-no-wrap">
                              <Link to={`/partner/reserv/allday/${reserv._id}`}>
                                {reserv.reservNumber}
                              </Link>
                            </p>
                          </td>
                                ...
                                <>
                                  <div className="mt-1 ">
                                    ...
                                    <Transition
                                      ...
                                      leaveTo="opacity-0"
                                    >
                                      <Listbox.Options className="absolute z-auto mt-1 w-80 bg-white shadow-lg max-h-56 rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm">
...
export default ReservAllDayTodayData;
