...
const SingleRestaurant = observer(() => {
  ...
  useEffect(() => {
    const getSingleRestaurant = async () => {
      await userStore.getInformationById(id);
      await userStore.getMenuByRest(id);
    };
    getSingleRestaurant();
  ...
  //-------reserv function
  const selfAllDayReserv = async (event) => {
    ...
    setStep(1);
    history.push(
      `/MyReservation/allday/general/${reservStore.thisReserv._id} `
    );
  };
  const customerAllDayReserv = async (event) => {
    event.preventDefault();
    ...
    setDate(new Date());
    setStep(1);
    history.push(`/MyReservation/allday/member/${reservStore.thisReserv._id} `);
  };
  const selfRoundReserv = async (event) => {
    event.preventDefault();
    ...
    setStep(1);
    history.push(
      `/MyReservation/rounds/general/${reservStore.thisReserv._id} `
    );
  };

  const customerRoundReserv = async (event) => {
    event.preventDefault();
    ...
    setDate(new Date());
    setStep(1);
    history.push(`/MyReservation/rounds/member/${reservStore.thisReserv._id} `);
  };
  ...
  if (!!reservModal) {
    reservElement = (
      <>
        {userStore.singleRestaurant.map((restaurant) => {
                      ...
                      <div className="bg-white px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                        {step >= 2 && (
                          <div className="grid grid-cols-2 sm:grid sm:grid-cols-2 md:grid md:grid-cols-2">
                            <div className="col-span-1 col-start-1 mt-4 flex justify-start  sm:flex sm:justify-start md:col-span-1 md:col-start-1">
                              {" "}
                              <button
                                disabled={step === 2}
                                className={
                                ...
                                <button
                                  ...
                                  onClick={(event) => {
                                    if (
                                      ...
                                      selfAllDayReserv(event);
                                      setStep(1);
                                    } else if (
                                      restaurant.type_rest === "rounds" &&
                                      !userStore.customer.username
                                    ) {
                                      selfRoundReserv(event);
                                      setStep(1);
                                    } else if (
                                      restaurant.type_rest === "allDay" &&
                                      userStore.customer.username
                                    ) {
                                      customerAllDayReserv(event);
                                      setStep(1);
                                    } else if (
                                      restaurant.type_rest === "rounds" &&
                                      userStore.customer.username
                                    ) {
                                      customerRoundReserv(event);
                                      setStep(1);
                                    }
